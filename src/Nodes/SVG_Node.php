 ...
 */
abstract class Svg_Node
{
    /**
     * @var SVGNodeContainer $parent The parent node.
     */
    protected ?Svg_Node_Container $parent;
    /**
     ...
     */
    public function get_parent(): ?Svg_Node_Container
    {
        return $this->parent;
    }
    /**
     ...
     */
    public function set_namespaces(array $namespaces): void
    {
     ...
     */
    public function set_value(?string $value): Svg_Node
    {
        if (!isset($value)) {
            unset($this->value);
            return $this;
     ...
     */
    public function get_style(string $name): ?string
    {
     ...
     * @return $this This node instance, for call chaining.
     */
    public function set_style(string $name, $value): Svg_Node
    {
     ...
     * @return $this This node instance, for call chaining.
     */
    public function remove_style(string $name): Svg_Node
    {
        unset($this->styles[$name]);
        return $this;
    }
    ...
    public function get_computed_style(string $name): ?string
    {
        ...
        if ($style === null && isset($this->parent)) {
            $container_styles = $this->parent->get_container_style_for_node($this);
            $style = $container_styles[$name] ?? null;
        }
        // If still no style then get parent's style
        if (($style === null || $style === 'inherit') && isset($this->parent)) {
            return $this->parent->get_computed_style($name);
        }
     ...
     * @return $this This node instance, for call chaining.
     */
    public function set_attribute(string $name, $value): Svg_Node
    {
        if (!isset($value)) {
     ...
     * @return $this This node instance, for call chaining.
     */
    public function remove_attribute(string $name): Svg_Node
    {
        unset($this->attributes[$name]);
        return $this;
    }
     ...
     */
    public function get_serializable_styles(): array
    {
        return $this->styles;
    }
    /**
     ...
     */
    public function get_id_and_class_pattern(): ?string
    {
        $id = $this->get_attribute('id') != null ? Str::trim($this->get_attribute('id')) : '';
        $class = $this->get_attribute('class') != null ? Str::trim($this->get_attribute('class')) : '';
        $pattern = '';
        if ($id !== '') {
            $pattern = '#' . $id . '|#' . $id;
        }
    ...
    {
        if ($this->get_attribute('viewBox') === null) {
            return null;
        }
        $attr = Str::trim($this->get_attribute('viewBox'));
     ...
     */
    abstract public function rasterize(Svg_Rasterizer $rasterizer): void;
    /**
